...
router.post('/nominate', authenticateToken, async (req, res) => {
  try {
    ...
    if (existingApplication) {
      // If pending or under review, inform user
      if (['pending', 'under_review'].includes(existingApplication.status)) {
        return res.status(400).json({ 
          error: 'This charity has already been nominated and is under review',
          status: existingApplication.status
        });
      }
      
      // If rejected, allow re-nomination
      if (existingApplication.status === 'rejected') {
        ...
        return res.status(201).json({
          message: 'Charity re-nominated successfully! We\'ll reach out to them.',
          application: {
