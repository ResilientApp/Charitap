...
export function AuthProvider({ children }) {
        ...
        window.chrome.runtime.sendMessage(
          ...
          (response) => {
            if (window.chrome.runtime.lastError) {
              console.error(`Website: Error sending message to extension ${process.env.REACT_APP_CHROME_EXTENSION_ID}: ${window.chrome.runtime.lastError.message}`);
  ...
  const checkEmailExists = (email, provider) => {
    const users = JSON.parse(localStorage.getItem('charitap_users') || '{}');
    const googleUsers = JSON.parse(localStorage.getItem('charitap_google_users') || '{}');
    
    // Check if email exists in either local or Google users
    const localUser = users[email];
    const googleUser = googleUsers[email];
    
    if (localUser && provider === 'google') {
      throw new Error('This email is already registered with email/password. Please use email/password login or use a different email.');
    }
    
    if (googleUser && provider === 'local') {
      throw new Error('This email is already registered with Google. Please use Google sign-in or use a different email.');
    }
    ...
    return {
      ...
      loginWithGoogle: async () => {
        ...
        try {
          ...
          checkEmailExists(info.email, 'google');
          ...
          // Store Google user separately
          const googleUsers = JSON.parse(localStorage.getItem('charitap_google_users') || '{}');
          googleUsers[info.email] = { id: info.id, email: info.email, firstName: info.firstName, lastName: info.lastName, provider: 'google', verified: true };
          localStorage.setItem('charitap_google_users', JSON.stringify(googleUsers));
